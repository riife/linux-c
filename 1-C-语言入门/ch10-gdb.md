...
1. 用 `gdb` 一步一步跟踪[第 5 章「深入理解函数」第 3 节「递归」](1-C-语言入门/ch05-深入理解函数#_3-递归)讲的 `factorial` 函数，对照着[第 5 章「深入理解函数」第 3 节「递归」图 5.2 「factorial (3) 的调用过程」](1-C-语言入门/ch05-深入理解函数#c5-2)查看各层栈帧的变化情况，练习本节所学的各种 `gdb` 命令。
